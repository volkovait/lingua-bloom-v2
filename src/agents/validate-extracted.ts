...
- Количество вопросов НЕ ДОЛЖНО превышать число кандидатов.
- Не создавай второе упражнение ради разнообразия типов — группируй по inputKind, но сохраняй все кандидаты.
...
- Кандидаты kind="gap" или "bracketGap" → gapFill (или gapDrag, если в материале явно есть банк слов). kind="mcq" → radio. kind="trueFalse" → radio с вариантами Верно/Неверно.
...
export async function validateExtractedPart(input: {
  ...
  const user = [
    ...
    `Число кандидатов (максимум вопросов): ${input.candidates.length}`,
    '',
